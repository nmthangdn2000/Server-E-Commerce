...
const updateById = async (id, name) => {
  // if (!id || !name) throw new Error(ERROR.CanNotUpdateProduct);
  // const slug = changeToSlug(name);
  // const update = await ProductModel.updateOne({ _id: id }, { name, slug, updatedAt: new Date() });
  // if (update.modifiedCount < 1) throw new Error(ERROR.CanNotUpdateProduct);
};
